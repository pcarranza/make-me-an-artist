...
describe "CommitPlanning weeks" do
  let(:initial_date) do
  ...
  let(:fifty_three_weeks_plan) do
    ...
    7.times do |day_of_week|
      weeks = []
      53.times do |week_of_year| weeks << week_of_year end
      days << weeks
    end
    CommitPlan.new(days)
  end
  ...
  let(:fifty_three_weeks_contributions) do
    ...
    (0..7 * 53).each do |offset|
      contributions << Contribution.new((rand * 10).floor, (initial_date + offset).to_s)
    end
...
describe CommitPlanning do
  ...
  context "with simple contributions" do
    let(:contributions) do
      contributions_list = []
    ...
    it "finds the right week" do
      planning = CommitPlanning.new(plan: one_week_plan, contributions: contributions)
    end
